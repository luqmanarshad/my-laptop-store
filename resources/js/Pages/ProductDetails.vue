                    ...
                    <div class="price-box mb-4 p-3 p-md-4 rounded-4 bg-light border fade-in-up" style="animation-delay: 0.2s;">
                        <template v-if="product.sale_price">
                            <div class="d-flex align-items-center flex-wrap gap-2 gap-md-3">
                                <span class="fs-1 fw-extrabold text-primary">${{ product.sale_price }}</span>
                                <span class="text-decoration-line-through text-muted fs-5">${{ product.price }}</span>
                                <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2 py-1 rounded">Save ${{ (product.price - product.sale_price).toFixed(2) }}</span>
                            </div>
                        </template>
                        <template v-else>
                            <span class="fs-1 fw-extrabold text-primary">${{ product.price }}</span>
                        </template>
                    </div>
